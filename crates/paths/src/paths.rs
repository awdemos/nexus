//! Paths to locations used by Nexus.
...
/// On macOS, this is `~/Library/Application Support/Nexus`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/nexus`.
/// On Windows, this is `%LOCALAPPDATA%\Nexus`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/nexus`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/nexus`.
/// On Windows, this is `%APPDATA%\Nexus`.
...
/// Returns the path to the configuration directory used by Nexus.
...
/// Returns the path to the data directory used by Nexus.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Nexus")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
pub fn state_dir() -> &'static PathBuf {
    ...
    STATE_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            return home_dir().join(".local").join("state").join("Nexus");
        }
...
/// Returns the path to the temp directory used by Nexus.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("nexus")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Nexus")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Nexus server directory on this SSH host.
...
/// Returns the path to the `Nexus.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Nexus.log"))
}

/// Returns the path to the `Nexus.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Nexus.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Nexus repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Nexus.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Nexus.
